...
/* This version differs from sparsemat.c in that we don't try to
   constrain ourselves to write-once modrefs. */

#ifndef __SPARSEMAT2_C__
#define __SPARSEMAT2_C__
...
typedef sparsevec_t sparsevec_tl_t ;
...
static sparsevec_tl_t** sparsevec_tls(long rowc, sparsevec_tl_t* rowv) {
  sparsevec_tl_t** tls;
  
#define FIXED 0
#if FIXED
  /* TODO: Parsing support for non-constant-sized arrays */
  tls = alloc(sparsevec_tl_t*[rowc]);
  for(long i = 0; i < rowc; i++) {
    tls[i] = &(rowv[i]);
  }
#else
  tls = malloc(sizeof(sparsevec_tl_t*) * rowc);
  memset(tls, 0, sizeof(sparsevec_tl_t*) * rowc);
#endif     
...
static sparsemat_t sparsemat_transpose(sparsemat_t m_in) {
  ...
  sparsevec_tl_t** tls = sparsevec_tls( m_out->rowc, m_out->rowv );
  ...
  /* All tails written with NULL. */
  for(long i = 0; i < m_out->rowc; i++) {
    *( tls[i] ) = NULL;
  }
...
static void sparsemat_vec_mult(sparsemat_t a,
                               sparsevec_t v,
                               sparsevec_tl_t* out)
{
  sparsevec_tl_t** out_tl = &out;
...
static sparsemat_t sparsemat_mat_mult(sparsemat_t a, sparsemat_t b) {
  ...
  for(long i = 0; i < b->colc; i++) {
    cut {
      ...
      sparsemat_vec_mult(a, c->rowv[i], &( d->rowv[i] ));
    }
  }
