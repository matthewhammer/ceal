/*
 *    monitor - Communicates with the wrapper on the platform and polls the wattmeter while the experiment is running
 *
 *   Based on:
...
static const char * prog_name = "monitor";
...
/* wu_data: the order here is the same as the order printed without command-line options.    --joshua */

/* the min and max here seem to be *for one  interval*, not over the whole log, unlike the numbers displayed on the device --joshua */
struct wu_data {
        unsigned int    watts;
        unsigned int    volts;
        unsigned int    amps;
        ...
        unsigned int    hertz;
        unsigned int    apparent_power;   /* volt amps */
};
...
static struct wu_field wu_fields[wu_num_fields] = {
        ...
        [wu_field_apparent_power]  = {
                .name   = "apparent-power",
                .descr  = "Volt amps (Apparent Power)",
        },
...
static int get_microseconds_hand (void)
{
  struct timeval tv;
  struct timezone tz;
  gettimeofday(&tv, &tz);
...
static int wu_read_data(int fd)
{
        ...
        i = 0;
        microseconds_hand = get_microseconds_hand();
        while (1) {
                ret = wu_read(fd, &p);
                if (ret) {
                ...
                int sleep_value = 1000000 - (get_microseconds_hand() - microseconds_hand);
                //                microseconds_hand = get_microseconds_hand();
                if (sleep_value < 960000) sleep_value = 960000;
                usleep(sleep_value);
...
static int wu_store_interval(int fd)
{
        ...
        if (!s) {
          err("wu_store_interval: wu_option_value(wu_option_interval) returned NULL");
          return ret_err(EINVAL);
        }

        dbg("wu_store_interval: s = %p", s);
        dbg("wu_store_interval: s -> %s", s);
